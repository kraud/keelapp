...
export const TagDataForm = (props: TagDataFormProps) => {
  ...
  const { isLoadingTags, tagLabelIsAlreadyInUse } = useSelector(
  ...
  async function checkLabelAvailability(label: string) {
    if (label.length > 2) {
      try {
        return await tagService.checkIfTagLabelAvailable(
          { tagLabel: label, userId: user._id },
          user.token,
        );
      } catch (error) {
  ...
  const validationSchema = Yup.object().shape({
    public: Yup.string()
      .required(t("tagDataForm.errors.publicRequired", { ns: "tags" }))
      .oneOf(
        ["Public", "Private", "Friends-Only"],
        t("tagDataForm.errors.publicNotMatch", { ns: "tags" }),
      ),
    label: Yup.string()
      .required(t("tagDataForm.errors.labelRequired", { ns: "tags" }))
      .matches(
        /^\S+$/,
        t("tagDataForm.errors.labelNotMatchingSpaces", { ns: "tags" }),
      )
  ...
  const [tagPublic, setTagPublic] = useState<
    "Public" | "Private" | "Friends-Only" | ""
  >("");
  ...
  useEffect(() => {
    if (props.currentTagData!! && props.currentTagData._id!!) {
      setValue("public", props.currentTagData.public, {
        shouldValidate: true,
        shouldTouch: true,
      });
      setTagPublic(
        props.currentTagData.public as
          "Public" | "Private" | "Friends-Only" | "",
      );
      setValue("label", props.currentTagData.label, {
        shouldValidate: true,
        shouldTouch: true,
      });
      setTagLabel(props.currentTagData.label);
      setValue("description", props.currentTagData.description, {
        shouldValidate: true,
        shouldTouch: true,
      });
  ...
  const filterAlreadySelectedOptions = (
    rawOptions: SearchResult[],
    selectedOptions: WordDataBE[],
  ) => {
    const selectedOptionsIds = selectedOptions.map(
      (selectedOption: WordDataBE) => {
        //@ts-ignore
  ...
  return (
    <Grid item={true} container={true} rowSpacing={2}>
            ...
            >
              ...
              props.currentTagData.words.length > 0
                ? props.currentTagData.words.length > 1
                  ? t("amountWord", {
                      ns: "tags",
                      count: props.currentTagData.words.length,
                    })
                  : t("amountWord", {
                      ns: "tags",
                      count: props.currentTagData.words.length,
            ...
            onSelect={(selectedWordItem: SearchResult) => {
              ...
              if (selectedWordItem.type === "word") {
                setSelectedWords((prevSelectedWordsState: WordDataBE[]) => [
                  ...prevSelectedWordsState,
                  selectedWordItem.completeWordInfo as WordDataBE,
                ]);
              }
            }}
        ...
        {selectedWords.map((selectedWordItem: WordDataBE, index: number) => {
          ...
          const wordDataToDisplay: WordChipDisplayData =
            getWordChipDataByLangInOrder(
              selectedWordItem,
              user.languages,
            ) as WordChipDisplayData;
          return (
            <Grid item={true} key={index.toString() + "-" + selectedWordItem}>
              <Chip
                variant="filled"
                label={
                  <>
