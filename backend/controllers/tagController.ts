...
const queryParamToBool = (value: unknown) => ((value + '').toLowerCase() === 'true');

const isUuid = (value: unknown) =>
    typeof value === 'string' &&
    /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value);

const normalizeTag = (tag: TagRow & { words?: WordRow[] }) => ({
    ...tag,
    _id: tag.id,
    author: tag.authorId,
    words: tag.words || [],
...
const getWordsByIds = async (wordIds: string[]) => {
...
const getTagsByIdsWithWords = async (tagIds: string[]) => {
    ...
    const wordRows = await getWordsByIds(wordIds);
...
const getTagDataByRequest = async (req: any, tagForceRequest?: any) => {
    ...
    if (!tagForceRequest) {
        if (query.id !== undefined) {
            if (!isUuid(query.id)) throw new Error('Tag auxiliary function getTagDataByRequest failed');
            filters.push(eq(tags.id, query.id));
        }
        if (query.author !== undefined) {
            if (!isUuid(query.author)) throw new Error('Tag auxiliary function getTagDataByRequest failed');
...
const searchTags = asyncHandler(async (req: any, res: any) => {
    ...
    const tagFilter = req.query.query ? ilike(tags.label, `%${req.query.query}%`) : undefined;
    ...
    if (includeOtherUserTags) {
        ...
        const friendUserIds = friendRows
            .filter((friendship) => friendship.user1Id === req.user.id || friendship.user2Id === req.user.id)
            .map((friendship) => (friendship.user1Id === req.user.id ? friendship.user2Id : friendship.user1Id));

        const allowedRows = await db.select({ id: tags.id }).from(tags).where(or(
            authorIsCurrentUser,
            eq(tags.public, 'Public'),
            and(eq(tags.public, 'Friends-Only'), inArray(tags.authorId, friendUserIds)),
            matchingTagsId.length > 0 ? inArray(tags.id, matchingTagsId) : sql`false`,
        ));
        ...
        const allowedRows = await db.select({ id: tags.id }).from(tags).where(or(
            authorIsCurrentUser,
            includeFollowedTags && matchingTagsId.length > 0 ? inArray(tags.id, matchingTagsId) : sql`false`,
        ));
        allowedTagIds = allowedRows.map((row) => row.id);
    }
    ...
    const searchResultTags = tagRows
        .filter((tagRow) => !tagFilter || String(tagRow.label).toLowerCase().includes(String(req.query.query).toLowerCase()))
        .map((tagData) => ({
            id: tagData._id,
            label: tagData.label,
            type: 'tag',
...
const getUserTags = asyncHandler(async (req: any, res: any) => {
    const tagsData = await getTagDataByRequest({ query: { author: req.user.id } });
...
const getOtherUserTags = asyncHandler(async (req: any, res: any) => {
    ...
    const usersAreFriends = friendRows.some((friendship) =>
    ...
    const whereClause = usersAreFriends
        ? and(eq(tags.authorId, userId), inArray(tags.public, ['Public', 'Friends-Only']))
...
const addTagsInBulkToWords = asyncHandler(async (req: any, res: any) => {
    ...
    const response = await Promise.all(tagIds.map(async (tagIdToAssign) => {
        const existingTagWordRows = await db
            .select({ wordId: tagWords.wordId })
            .from(tagWords)
            .where(eq(tagWords.tagId, tagIdToAssign));

        const allWordIdsRelatedToTag = existingTagWordRows.map((row) => row.wordId);
...
const addExternalTag = asyncHandler(async (req: any, res: any) => {
    ...
    const [tagData] = await db.select().from(tags).where(eq(tags.id, tagId)).limit(1);
    if (!tagData) {
        res.status(400);
        throw new Error('Tag not found');
    }
    ...
    const clonedWordRows = await Promise.all(originalWords.map(async (originalWordItem) => {
    ...
    const tagWordResponse = newTagWordList.length > 0
...
const deleteTag = asyncHandler(async (req: any, res: any) => {
    ...
    await db.delete(userFollowingTags).where(eq(userFollowingTags.tagId, req.params.id));
...
const deleteUserFollowingTag = asyncHandler(async (req: any, res: any) => {
    ...
    const [userFollowingTag] = await db
        .select()
        .from(userFollowingTags)
        .where(and(eq(userFollowingTags.tagId, tagId), eq(userFollowingTags.followerUserId, userId)))
        .limit(1);
...
const updateTag = asyncHandler(async (req: any, res: any) => {
    ...
    const updatedWordsList = (req.body.words || []).map((wordFullData: any) => wordFullData._id);
    const existingTagWordRows = await db
        .select({ wordId: tagWords.wordId })
        .from(tagWords)
    ...
    const wordsToBeRemoved = existingWords.filter((wordId) => !updatedWordsList.includes(wordId));
    const wordsToBeAdded = updatedWordsList.filter((wordId) => !existingWords.includes(wordId));

    if (wordsToBeRemoved.length > 0) {
        await db.delete(tagWords).where(and(eq(tagWords.tagId, req.params.id), inArray(tagWords.wordId, wordsToBeRemoved)));
    }
    if (wordsToBeAdded.length > 0) {
        await db.insert(tagWords).values(wordsToBeAdded.map((wordId) => ({
            tagId: req.params.id,
            wordId,
        })));
    }
