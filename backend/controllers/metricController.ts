...
interface BasicUserMetrics {
    ...
    translationsByLanguageAndPOS: Array<{ label: string; type: string; partOfSpeech: string; count: number }>;
 ...
 */
const calculateBasicUserMetrics = async (user: { _id: string; languages?: string[] }): Promise<BasicUserMetrics | undefined> => {
    const userId = user._id;
    const userLanguages: string[] = user.languages !== undefined ? user.languages : [];
    ...
    const translationsByLanguage = transByLangRows.map((r) => ({
        language: r.language,
        count: r.count,
        type: 'language',
    }));
    ...
    const translationsByLanguageAndPOS = transByLangPosRows.map((r) => ({
        label: r.language,
        type: 'language',
        partOfSpeech: r.partOfSpeech,
        count: r.count,
    ...
    const wordsPerMonthRows = await db
        ...
        .orderBy(sql`EXTRACT(YEAR FROM ${words.createdAt})`, sql`EXTRACT(MONTH FROM ${words.createdAt})`);
