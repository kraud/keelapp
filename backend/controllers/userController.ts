...
const serializeLoginUser = (user: UserRow) => ({
    ...
    nativeLanguage: user.nativeLanguage === null ? undefined : user.nativeLanguage,
...
const registerUser = asyncHandler(async (req: any, res: any) => {
    ...
    await sendMail({ email: user.email, subject: 'Verify Email', url, name: user.name, type: 'verifyEmail' });
    ...
    if (requestQuery.searchOnlyFriends) {
        ...
        const matchingFriendships = await db
            ...
            .where(and(
                ...
                or(eq(friendships.user1Id, req.user.id), eq(friendships.user2Id, req.user.id)),
            ));

        matchingFriendIds = matchingFriendships.map((friendship: any) =>
            friendship.user1Id === req.user.id ? friendship.user2Id : friendship.user1Id,
        );
...
const verifyUser = asyncHandler(async (req: any, res: any) => {
    try {
        ...
        const [token] = await db
            .select()
            .from(tokens)
            .where(and(eq(tokens.userId, user.id), eq(tokens.token, req.params.token)))
            .limit(1);
...
const requestPasswordReset = asyncHandler(async (req: any, res: any) => {
    ...
    try {
        ...
        await sendMail({ email: user.email, subject: 'Password reset link', url, name: user.name, type: 'resetPassword' });
...
const updatePassword = asyncHandler(async (req: any, res: any) => {
    ...
    await db
        .update(users)
        .set({ password: hashedPassword, passwordTokens: [], updatedAt: new Date() })
        .where(eq(users.id, user.id));
...
const getBasicUserMetrics = asyncHandler(async (req: any, res: any) => {
    ...
    // Count words owned by the user; this replaces the Mongo aggregation totalWords facet.
    const [totalWordsRow] = await db
        .select({ count: sql<number>`count(*)::int` })
        .from(words)
        .where(eq(words.userId, user.id));

    // Count words missing at least one of the user's selected translation languages.
    const userLanguages: string[] = user.languages || [];
    const wordRows = await db.select({ id: words.id }).from(words).where(eq(words.userId, user.id));
    const incompleteWords = await Promise.all(wordRows.map(async (word: any) => {
        const languageRows = await db
            .select({ language: translations.language })
            .from(translations)
            .where(eq(translations.wordId, word.id));
        const translatedLanguages = new Set(languageRows.map((row: any) => row.language));
        return userLanguages.every((language) => translatedLanguages.has(language)) ? 0 : 1;
    }));

    // Keep the response shape expected by the dashboard while later phases migrate richer metrics.
    const incompleteWordsCount = incompleteWords.reduce<number>((sum, count) => sum + count, 0);
