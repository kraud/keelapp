...
/**
 * The front-end expects translations as a nested array (legacy shape).
 * This internal type represents one assembled translation unit.
 */
interface AssembledTranslation {
    _id: string;
    language: string;
 ...
 * Every public endpoint returns words in this format.
 */
...
// ---------------------------------------------------------------------------
// HELPERS: Build the legacy nested response shape
// ---------------------------------------------------------------------------

/**
 * Fetch all translations + cases for a set of word UUIDs.
 * Returns a Map<wordId, AssembledTranslation[]> for O(1) look-up.
 */
const fetchTranslationsMap = async (wordIds: string[]): Promise<Map<string, AssembledTranslation[]>> => {
...
/**
 * Fetch all tags linked to a set of word UUIDs via the tag_words junction.
 * Returns a Map<wordId, TagRow[]>.
 */
const fetchTagsMap = async (wordIds: string[]): Promise<Map<string, TagRow[]>> => {
    if (wordIds.length === 0) return new Map();
    ...
    const tagIds = [...new Set(junctionRows.map((j) => j.tagId))];
...
/**
 * Assemble a single WordResponse from a WordRow plus the pre-fetched maps.
 */
const assembleWord = (
    word: WordRow,
...
/**
 * Fetch one or more words (by id array) and return them as fully assembled
 * WordResponse objects (with nested translations, cases, and tags).
 */
const fetchWordsWithRelations = async (wordIds: string[]): Promise<WordResponse[]> => {
 ...
 */
const fetchWordWithRelations = async (wordId: string): Promise<WordResponse | null> => {
...
const buildWordConditions = (query: Record<string, any>): any[] => {
    ...
    if (query.partOfSpeech !== undefined) conditions.push(eq(words.partOfSpeech, query.partOfSpeech));
    if (query.clue !== undefined) conditions.push(ilike(words.clue, `%${query.clue}%`));
 ...
 */
const getWordsByTagFiltering = async (tagFilters: Array<{ _id: string }>): Promise<WordResponse[]> => {
 ...
 */
const getRequiredFieldsData = (translation: AssembledTranslation, partOfSpeech: string): Record<string, string> => {
    ...
    switch (partOfSpeech) {
        case 'Noun': {
            switch (translation.language) {
                case 'Estonian': return { dataEE: findByCaseName('singularNimetavEE')! };
                case 'English':  return { dataEN: findByCaseName('singularEN')! };
                case 'Spanish':  return {
                ...
                default: throw new Error('Language not found for this part of speech (Noun)');
            }
        }
        case 'Adverb': {
            switch (translation.language) {
                case 'English': return { dataEN: findByCaseName('adverbEN')! };
                case 'Spanish': return { dataES: findByCaseName('adverbES')! };
                case 'German':  return { dataDE: findByCaseName('adverbDE')! };
                default: throw new Error('Language not found for this part of speech (Adverb)');
            }
        }
        case 'Adjective': {
            switch (translation.language) {
                case 'English': return { dataEN: findByCaseName('positiveEN')! };
                case 'Spanish': {
                    const male = findByCaseName('maleSingularES');
                    if (male) return { dataES: male };
                    return { dataES: findByCaseName('neutralSingularES')! };
                }
                case 'German':   return { dataDE: findByCaseName('positiveDE')! };
                case 'Estonian': return { dataEE: findByCaseName('algvorreEE')! };
                default: throw new Error('Language not found for this part of speech (Adjective)');
            }
        }
        case 'Verb': {
            switch (translation.language) {
                case 'English': return { dataEN: findByCaseName('simplePresent1sEN')! };
                case 'Spanish': return { dataES: findByCaseName('infinitiveNonFiniteSimpleES')! };
                case 'Estonian': return { dataEE: findByCaseName('infinitiveMaEE')! };
                case 'German':   return { dataDE: findByCaseName('infinitiveDE')! };
                default: throw new Error('Language not found for this part of speech (Verb)');
            }
        }
        default: throw new Error('Part of speech not found');
    }
};
    ...
    for (const raw of filters) {
        ...
        if (idx === -1) {
            ...
            if (raw.type === 'tag') {
                ...
                merged.push({ type: raw.type, filterValue: raw.restrictiveArray || [] });
            } else {
                merged.push({ type: raw.type, filterValue: raw.filterValue !== undefined ? [raw.filterValue] : [] });
            }
        } else {
 ...
 */
const diffTagWords = async (wordId: string, incomingTags: Array<{ _id: string }>): Promise<{ toRemove: string[]; toAdd: string[] }> => {
...
const getWords = asyncHandler(async (req: any, res: any) => {
    ...
    const result = wordRows.map((w) => assembleWord(w, tMap, tagsMap));
...
const getWordsSimplified = asyncHandler(async (req: any, res: any) => {
    ...
    const followedTagWordIds = await getWordsIdFromFollowedTagsByUserId(req.user.id);

    const rawFilters: RawFilter[] = req.query.filters !== undefined ? req.query.filters : [];
    ...
    const accessCondition = or(
        ...
        followedTagWordIds.length > 0 ? inArray(words.id, followedTagWordIds) : sql`false`,
    );
        ...
        for (const filter of mergedFilters) {
            if (filter.type === 'tag') {
                ...
                const rows = await db
                    .select()
                    .from(words)
                    .where(and(inArray(words.partOfSpeech, filter.filterValue), accessCondition));
                ...
                const matchingRows = await db
                    ...
                    .innerJoin(translationCases, eq(translations.id, translationCases.translationId))
                    .where(
                        and(
...
const setWord = asyncHandler(async (req: any, res: any) => {
    ...
    const newTranslations = await db.insert(translations).values(translationInserts).returning();

    // 3. Insert case rows for each translation
    const caseInserts: Array<{ translationId: string; caseName: string; word: string }> = [];
    for (let i = 0; i < newTranslations.length; i++) {
        const tCases = req.body.translations[i].cases || [];
        for (const c of tCases) {
            caseInserts.push({ translationId: newTranslations[i].id, caseName: c.caseName, word: c.word });
        }
    }
...
const updateWord = asyncHandler(async (req: any, res: any) => {
    ...
    await db.delete(translationCases).where(
        inArray(
            translationCases.translationId,
            db.select({ id: translations.id }).from(translations).where(eq(translations.wordId, req.params.id)),
        ),
    );
    ...
    const newTranslations = req.body.translations
        ? await db
              .insert(translations)
              .values(
                  req.body.translations.map((t: any) => ({ wordId: req.params.id, language: t.language })),
              )
              .returning()
        : [];
    ...
    if (toRemove.length > 0) {
        await db
            .delete(tagWords)
            .where(and(eq(tagWords.wordId, req.params.id), inArray(tagWords.tagId, toRemove)));
    }
    if (toAdd.length > 0) {
        await db.insert(tagWords).values(toAdd.map((tagId) => ({ tagId, wordId: req.params.id })));
    }
...
const filterWordByAnyTranslation = asyncHandler(async (req: any, res: any) => {
    ...
    const matchingCaseRows = await db
        ...
        .where(
            and(
                ...
                or(
                    eq(words.userId, req.user.id),
                    wordsFromFollowedTags.length > 0 ? inArray(words.id, wordsFromFollowedTags) : sql`false`,
                ),
            ),
        );
    ...
    const wordMap = new Map((await fetchWordsWithRelations(wordIds)).map((w) => [w.id, w]));
...
const getWordDataByRequest = async (
    ...
    try {
            ...
            for (const f of wordForceRequest) {
                if (f.$or) {
                    ...
                    for (const orItem of f.$or) {
                        ...
                        if (orItem._id?.$in) orParts.push(inArray(words.id, orItem._id.$in.map(String)));
                    }
        ...
        const conditions = wordRequest ? buildWordConditions(wordRequest.query) : [];
        const where = conditions.length > 0 ? and(...conditions) : undefined;
        const rows = await db.select().from(words).where(where);
        const ids = rows.map((r) => r.id);
        return fetchWordsWithRelations(ids);
